...
**VS Code extension 和 terminal 的差別在這裡很重要。** 如果你用的是 VS Code 裡面的 Claude Code extension，language server 已經在跑了——VS Code 會自動啟動它們（TypeScript 用 tsserver，Python 用 Pylance/pylsp）。只要開啟上面那個 flag，LSP 就直接可用。

如果你用的是 terminal 裡面的 Claude Code，你需要自己安裝 language server：
...
簡單來說：VS Code 免費送你 LSP。Terminal 需要一步安裝。兩邊都要開啟那個 flag。
...
你可以手寫 AI_INDEX.md，但只要 repo 有超過幾個檔案，直接跑 `/generate-index`。它掃描你的 source directory，找到每一個 import，然後輸出整個 routing manifest，所有 `連接至` 的 edges 都填好了。然後 review output——加 HTTP endpoints、合併應該放在一起的 domains、去掉雜訊。
